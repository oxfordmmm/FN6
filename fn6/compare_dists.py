...
def main():
    parser = argparse.ArgumentParser(
        description="Compare two distance files for functional equality"
    )
    parser.add_argument("file1", help="First distance file", required=True)
    parser.add_argument("file2", help="Second distance file", required=True)
    args = parser.parse_args()
    with open(args.file1, "r") as f:
        dists1 = [line.strip().split(" ") for line in f]
        dists1 = {
            (sorted([s1, s2])[0], sorted([s1, s2])[1], dist) for s1, s2, dist in dists1
        }

    with open(args.file2, "r") as f:
        dists2 = [line.strip().split(" ") for line in f]
        dists2 = {
            (sorted([s1, s2])[0], sorted([s1, s2])[1], dist) for s1, s2, dist in dists2
        }
